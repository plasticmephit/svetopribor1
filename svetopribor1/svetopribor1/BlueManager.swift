...
class BluetoothManager: NSObject, CBPeripheralManagerDelegate, CBCentralManagerDelegate, CBPeripheralDelegate, UIDocumentPickerDelegate {
    ...
    func sendCurrentPacket() {
        ...
        guard !shouldStopUpdate, isUpdating else {
        ...
        // Убедимся, что характеристика для обновления доступна
        ...
        let crcString = formatCRC32ToMAC(crc32: crc32Mac) // Формирование строки CRC
        ...
        // Отправляем данные, ожидая ответ через .withResponse
    ...
    func peripheral(_ peripheral: CBPeripheral, didWriteValueFor characteristic: CBCharacteristic, error: Error?) {
        ...
        // Если еще остались пакеты, планируем отправку следующего пакета с небольшой задержкой
        if currentPacketIndex < packets.count {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.0) { [weak self] in
                guard let self = self, self.isUpdating, !self.shouldStopUpdate else {
